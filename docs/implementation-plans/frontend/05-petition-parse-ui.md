...
- 409 when the evaluation id is unknown to this session (uniform 404 semantics from doc 01 apply first).
  (Amended 2026-08-20 and 2026-08-21: a POST for a selection whose petition is still pending, or already succeeded without fallback, no longer starts a new job; it returns 202 with the existing `petition_id`, per the decision 6 amendments in `../llm-nodes/00-overview.md`.
  Re-checking a combination therefore replays its letter in about one poll interval.)
