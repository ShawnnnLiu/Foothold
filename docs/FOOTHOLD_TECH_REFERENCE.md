# Foothold Technical Reference (from Agentic-Calendar)

PIVOT NOTICE (2026-07-31): the product pivoted to the transfer credit navigator (then named Astrolabe, renamed to Foothold on 2026-08-01); see `FOOTHOLD_PATHFINDERS_PLAN.md`.
This file was renamed from `STARMAP_TECH_REFERENCE.md` in the 2026-08-01 rename.
...
This document is the written design record the Foothold build agent works from.
...
Read it together with `FOOTHOLD_PATHFINDERS_PLAN.md` (the product/architecture plan); this file is the mechanism-level companion.

Rules of engagement:

- Contest terms forbid copying code from Agentic-Calendar; every line in the Foothold repo must be newly written.
...
- Every section ends with a "Foothold deltas" note saying what to drop, shrink, or change for Foothold.
...
- Corpus documents = one per course (title + description + prereq prose + fulfills notes) and one per requirement group, generated by the catalog build; there is no web manifest, because the catalog build IS the ingest (fetch policy concerns move to the bulletin fetcher).
  Foothold's planned 1 req/s politeness is NEW code; do not assume the reference has a rate limiter to copy.
...
### Foothold deltas (pathway map) - including the flagged design change

Foothold ADDS two things this map deliberately lacks: semester ordering and prerequisite edges.
...
- Drop: mastery tiers, overlays/additions/personal content, capstones, evidence slots, groups (Foothold's grouping axis is the pathway tab + semester column; department or requirement-group could color nodes instead), and all account-map merging.
- Keep the vocabulary-gate pattern in its Foothold form: the candidate course pool that goes into the proposer prompt IS the list the validator checks `unknown_course` against - one projection, two consumers, same object.
...
Loop's resume-extraction merge policy, worth copying for any auto-fill Foothold adds later:
...
### 3.5 The profile contract, with the Foothold keep/drop map

`UserProfile` (frozen, extra=forbid) fields, annotated:

| Field | Constraint | Foothold |
|---|---|---|
...
| `pathway_selection` | nullable nested model | drop (Foothold generates per-request) |
...
- Keep wholesale: the React-free wizard module with the exported-surface shape above, the clamp deep-link policy, the `''`-vs-null payload discipline, `cleanList`, the drop-title-less-rows rule, the prefill-with-defaults vs preserve-existing split, and the session-user-overrides-client-user trust boundary (Foothold: the `sid` cookie user).
...
- Keep `common/sqlite.py` essentially as specified in 4.3; it is small and exactly right for one-machine SQLite.
  Foothold has three databases: `catalog.db` and `corpus.db` read-only in the image, `sessions.db` (profiles, pathway sets, cache, call log) on the volume.
...
- Keep the contracts conventions wholesale (extra=forbid + frozen, rebuild-through-validate, validator message discipline, one spec doc per contract, generated schemas with `--check`), scaled to Foothold's ~10 contracts.
...
- Loop's ingest tool has NO sleep-based rate limiting (politeness = per-run cap + timeout + no retries + robots.txt); Foothold's planned 1 req/s bulletin fetcher is new code, not a port.
